    ...
    switch(p_toolType){
        ...
        case 2: // Displaying Enemies
            for(i = 0; i < p_gameMgr->allEnemiesSize; i++){
                ...
                if(i == p_selectedTool){
                    cursorPos.y = objectPos.y + p_gameMgr->allEnemies[i]->sprites[0][0]->h / 2;
                    cursorPos.y += p_cursorDecalY;
                    SDL_BlitSurface(p_cursor, NULL, SDL_GetWindowSurface(p_window), &cursorPos);
                }
                objectPos.y += p_gameMgr->allEnemies[i]->sprites[0][0]->h;
            }
            break;
    }
...
// Get selected tool Id
    ...
    if(p_clicX >= p_toolBoxStartX){
        ...
        case 2: //enemies
            for(i = 0; i < p_gameMgr->allEnemiesSize; i++){
                if(p_clicY > heightSum && p_clicY <= heightSum + p_gameMgr->allEnemies[i]->sprites[0][0]->h){
                    *p_selectedTool = i;
                    break;
                }
                heightSum += p_gameMgr->allEnemies[i]->sprites[0][0]->h;
            }
            break;
        }
    }
...
//--------------------------Events management-----------------------------------------------------------------------
        ...
        if(event.type == SDL_MOUSEBUTTONDOWN && event.button.button == SDL_BUTTON_RIGHT) {
            ...
            if(clicX < SDL_GetWindowSurface(p_window)->w - toolbox->w ){
                removeElementFromLevelByGameMgr(p_gameMgr, clicX + deplaX, SDL_GetWindowSurface(p_window)->h - clicY + deplaY);
            }
        }
