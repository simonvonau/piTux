...
void displayAllActivesColliders(SDL_Window *p_window, ColliderManager *p_collMgr, int deplaX, int deplaY){
// Display all the active collider (very useful for debugging)
    int i;
    Collider *currColl;
    SDL_Surface * collSurf;
    SDL_Rect pos, pos2;
    int screenW = SDL_GetWindowSurface(p_window)->w;
    int screenH = SDL_GetWindowSurface(p_window)->h;
    int screenLeft = deplaX - screenW / 2;
    int screenRight = deplaX + screenW / 2 + 400;

    for(i = 0; i < p_collMgr->allCollidersSize; i++){
        currColl = p_collMgr->allColliders[i];
        if(currColl->isEnabled && currColl->posX < screenRight && currColl->posX + currColl->width > screenLeft){

            collSurf = SDL_CreateRGBSurface(0, currColl->width, currColl->height, 32, 0, 0, 0, 0);
            pos.x = 0;
            pos.y = 0;
            pos.w = currColl->width;
            pos.h = currColl->height;
            SDL_FillRect(collSurf, &pos, SDL_MapRGBA(collSurf->format, 0, 255, 0, 255));
            pos2.x = 0;
            pos2.y = 0;
            pos2.x = currColl->posX - deplaX;
            pos2.y = screenH - currColl->posY - currColl->height + deplaY;
            SDL_BlitSurface(collSurf, NULL, SDL_GetWindowSurface(p_window), &pos2);
            SDL_FreeSurface(collSurf);
        }
    }
...
int displayGamePage(SDL_Window *p_window, char *p_levelPath, int p_levelPathSize, GameManager *p_gameMgr){
    ...
    //--------------------------------Laying out ---------------------------------------------------------------------------------------
        ...
        if(COLL_DRAWING_ENABLED){
            displayAllActivesColliders(p_window, p_gameMgr->collMgr, cameraX, cameraY);
        }
