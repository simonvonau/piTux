...
// Display the current level
    ...
    for( i = 0; i < p_lev->blockInstancesSize; i++){
        ...
        && p_lev->blockInstances[i]->posY >= bottomLimit && p_lev->blockInstances[i]->posY <= topLimit){
            ...
            currSprite = p_allBlocks[p_lev->blockInstances[i]->idBlock]->sprites[p_lev->blockInstances[i]->currentActionId][p_lev->blockInstances[i]->currentSpriteId];
            currColl = p_lev->blockInstances[i]->coll;

            objectPos.x = p_lev->blockInstances[i]->posX - p_deplaX;
            objectPos.y = SDL_GetWindowSurface(p_window)->h - p_lev->blockInstances[i]->posY - currColl->height + p_deplaY;
            SDL_BlitSurface(currSprite, NULL, SDL_GetWindowSurface(p_window), &objectPos);
    ...
    for( i = 0; i < p_lev->bonusInstancesSize; i++){
        ...
        &&p_lev->bonusInstances[i]->posY >= bottomLimit && p_lev->bonusInstances[i]->posY <= topLimit){
            ...
            currSprite = p_allBonus[p_lev->bonusInstances[i]->idBonus]->sprites[p_lev->bonusInstances[i]->currentActionId][p_lev->bonusInstances[i]->currentSpriteId];
            currColl = p_lev->bonusInstances[i]->coll;

            objectPos.x = p_lev->bonusInstances[i]->posX - p_deplaX;
            objectPos.y = SDL_GetWindowSurface(p_window)->h - p_lev->bonusInstances[i]->posY - currColl->height + p_deplaY;
            SDL_BlitSurface(currSprite, NULL, SDL_GetWindowSurface(p_window), &objectPos);
        }
    }
    ...
    for( i = 0; i < p_lev->enemyInstancesSize; i++){
        ...
        && p_lev->enemyInstances[i]->posY >= bottomLimit && p_lev->enemyInstances[i]->posY <= topLimit){
            ...
            currSprite = p_allEnemies[p_lev->enemyInstances[i]->idEnemy]->sprites[p_lev->enemyInstances[i]->currentActionId][p_lev->enemyInstances[i]->currentSpriteId];

            objectPos.x = p_lev->enemyInstances[i]->posX - p_deplaX;
            objectPos.y = SDL_GetWindowSurface(p_window)->h - p_lev->enemyInstances[i]->posY - currSprite->h + p_deplaY;
            SDL_BlitSurface(currSprite, NULL, SDL_GetWindowSurface(p_window), &objectPos);
        }
    }
    ...
    for( i = 0; i < p_lev->fireBulletInstancesSize; i++){
        ...
        &&p_lev->fireBulletInstances[i]->posY >= bottomLimit && p_lev->fireBulletInstances[i]->posY <= topLimit){
            ...
            currSprite = p_fireBullet->sprites[p_lev->fireBulletInstances[i]->currentSpriteId];
            currColl = p_lev->fireBulletInstances[i]->coll;

            objectPos.x = p_lev->fireBulletInstances[i]->posX - p_deplaX;
            objectPos.y = SDL_GetWindowSurface(p_window)->h - p_lev->fireBulletInstances[i]->posY - currColl->height + p_deplaY;
            SDL_BlitSurface(currSprite, NULL, SDL_GetWindowSurface(p_window), &objectPos);
        }
    }
