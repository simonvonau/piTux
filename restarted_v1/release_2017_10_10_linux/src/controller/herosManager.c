...
// Update the heros behaviour AFTER having some collisions
    ...
    int tuxFallY = 0; // When tux go down


    if(!p_herosInstance->isDead){
        tuxFallY = tux->posY - tux->lastPosY;
        ...
        for (i = 0; i < contactPointsSize; i++){
            switch(contactPoints[i]->ownerTag){
                ...
                case tag_explosion:
                    ...
                    break;
                default:
                    if(tuxFallY > 0){tuxFallY = 0;}
                    // Testing if the heros is above the colliding object
                    if( tux->lastPosY >= contactPoints[i]->posY + contactPoints[i]->height + tuxFallY){
                    ...
                    // Testing if the heros is under the colliding object
        ...
        for(i = 0; i < verticalCollId; i++){
            if(contactPoints[verticalColl[i]]->posX + contactPoints[verticalColl[i]]->width > leftWall
               ...
               // if Tux is above something
                if(contactPoints[verticalColl[i]]->posY < tux->lastPosY){
                    isAboveSomething = 1;
                    buffer = contactPoints[verticalColl[i]]->posY + contactPoints[verticalColl[i]]->height;
                    if(buffer > tux->posY){
                        tux->posY = buffer;
                    }
                // if Tux is under something
                }else{
        ...
        if(p_herosInstance->fireKeyPressed && p_herosInstance->currState == 2 && p_herosInstance->timeBeforeNextShot <= 0 && p_herosInstance->hasReleaseFireKey){
            ...
            if(p_herosInstance->lastDirection == 'r'){
                addBulletInstanceFromLevelMgr(p_collMgr, p_levMgr, tux->posX + tux->width + 10, tux->posY + tux->height / 2, p_fireBullet, 1);
                changeHerosAction(p_herosInstance, 7, 0);
            }else{
                addBulletInstanceFromLevelMgr(p_collMgr, p_levMgr, tux->posX - 10 - p_fireBullet->refColl->width, tux->posY + tux->height / 2, p_fireBullet, -1);
